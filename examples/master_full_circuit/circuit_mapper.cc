...
bool CircuitMapper::map_inline(Inline *inline_operation)
{
  ...
  //if (!first) {
  if (false) {
    Memory fb_mem = machine_interface.find_memory_kind(all_gpus[0],
                                                     Memory::GPU_FB_MEM);
    assert(fb_mem.exists());
    inline_operation->requirement.target_ranking.clear();
    inline_operation->requirement.target_ranking.push_back(fb_mem);
    first = true;
  } 
